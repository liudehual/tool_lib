...
public:
    ...
    void Add( T* entry ) {
        K key( entry );
        unsigned int theIndex = ComputeIndex( key.GetHashKey() );
    ...
    unsigned long long GetNumEntries() { return fNumEntries; }
    
    unsigned int GetTableSize() { return fSize; }
...
private:
    ...
    unsigned int fSize;
    unsigned int fMask;
    unsigned long long fNumEntries;
    
    unsigned int ComputeIndex( unsigned int hashKey )
    {
        if (fMask)
