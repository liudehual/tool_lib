...
void MultiFramedRTPSink::setPacketSizes(unsigned preferredPacketSize,
  ...
  fprintf(stderr,"preferredPacketSize %d maxPacketSize %d\n",preferredPacketSize, maxPacketSize);
...
void MultiFramedRTPSink::packFrame() {
  ...
  if (fOutBuf->haveOverflowData()) {
	...
	Medium::fThread->setThreadState(DO_NOTHING);
...
::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes,
  ...
  //fprintf(stderr,"Get Frame FrameSize %d\n",frameSize);
...
void MultiFramedRTPSink::sendPacketIfNecessary() {
  ...
  if (fNoFramesLeft) {
    ...
    //nextTask() = envir().taskScheduler().scheduleDelayedTask(1000*10, (TaskFunc*)sendNext, this);
	//fprintf(stderr,"%d \n",uSecondsToGo/1000);
	nextTask() = envir().taskScheduler().scheduleDelayedTask(1000, (TaskFunc*)sendNext, this);
