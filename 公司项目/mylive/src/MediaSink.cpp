...
Boolean MediaSink::startPlaying(MediaSource& source,
  ...
  fprintf(stderr,"startPlaying Run \n");
  ...
  if (!sourceIsCompatibleWithUs(source)) {
	  fprintf(stderr,"source is not a frame source \n");
	 return False;
  }
  #endif
  ...
  fprintf(stderr,"OK startPlaying Run \n");
...
OutPacketBuffer::OutPacketBuffer(unsigned preferredPacketSize,
  ...
  fprintf(stderr,"fLimit %d "
  					"maxNumPackets %u "
  					"maxPacketSize %d "
  					"OutPacketBuffer::maxSize  %u \n",
  					fLimit,
  					maxNumPackets,
  					maxPacketSize,
...
void OutPacketBuffer::enqueue(unsigned char const* from, unsigned numBytes) {
 ...
 // fprintf(stderr,"numBytes %d totalBytesAvailable() %d\n",numBytes,totalBytesAvailable());
