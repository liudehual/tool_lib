...
void Socket::SetBlockTime(UInt32 MillSeconds)
{
    struct timeval t;
    t.tv_sec = MillSeconds / 1000;
    t.tv_usec = (MillSeconds % 1000) * 1000;
    
    int err = ::setsockopt(fFileDesc, SOL_SOCKET, SO_SNDTIMEO, (const char*)&t, sizeof(t));
    printf("SetBlockTime: %d\n",errno);
    Assert(err == 0);  
