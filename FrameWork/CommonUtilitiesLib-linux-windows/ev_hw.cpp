...
{
    return select_modwatch_hw(req, which);
}
...
int select_waitevent_hw(struct eventreq *req, void* /*onlyForMacOSX*/)
{
    ...
    if (theFDsProcessed < sNumFDsBackFromSelect)
    {
        ...
        if (!sInReadSet)
        {
            ...
            {
                // This can happen if another thread calls select_removeevent_hw at just the right
                ...
                // by select_removeevent_hw anyway. We need to make sure to finish iterating over
    ...
    while(!selecthasdata())
    {
        ...
        if ( yieldDur > 1 )
        {
            qtss_printf( "select_waitevent_hw time in OSThread::Yield() %i, numZeroYields %i\n", (SInt32)yieldDur, (SInt32)numZeroYields );
            numZeroYields = 0;
        }
        else
            numZeroYields++;
...
bool selecthasdata()
{
    ...
    if (FD_ISSET(sPipes[0], &sReturnedReadSet))
    {
        ...
        // CISCO contribution
        char theBuffer[4096]; 
        ssize_t bytes=::read(sPipes[0], &theBuffer[0], 4096);
		if(bytes<0){
#if EV_DEBUGGING
					qtss_printf("read is error \n");
#endif
