...
void OS::Initialize()
{
    if(!Initialized)        //add by gyd 20150925 
    {
        ...
        ::select_startevents();
#ifdef __linux__
        ::select_startevents_epoll();
#endif
        TaskThreadPool::AddThreads(4);
        TaskThreadPool::SetNumBlockingTaskThreads(2);
        TaskThreadPool::SetNumShortTaskThreads(2);
        OSThread::Sleep(1000); //wait for all threads start
        ////start event select
        Socket::Initialize();
