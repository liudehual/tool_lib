...
void EventContext::Cleanup()
{
    ...
    if (fFileDesc != kInvalidFileDesc)
    {
        ...
        if (fUniqueID > 0)
        {
            ...
            if(fEventThread->IsforSocket())
            {
                if(!fEventThread->IsUseEpoll())
                {
            ...
            else
            {
                ...
                select_removeevent_hw(fFileDesc);//The eventqueue / select shim requires this
#endif
            }
#ifdef __Win32__
...
void EventContext::RequestEvent(int theMask)
{
    ...
    if (fWatchEventCalled)
    {
        ...
        if(fEventThread->IsforSocket())
        {
            ...
            {
               // printf("select_modwatch_epoll\n");
                if (select_modwatch_epoll(&fEventReq, theMask) != 0)
                    AssertV(false, OSThread::GetErrno());
            }
#endif
        }
        ...
        {
#ifdef __linux__
             if (select_modwatch_hw(&fEventReq, theMask) != 0)
                AssertV(false, OSThread::GetErrno());
#endif
        }
    }
...
#endif
		#ifdef PRINT
        //printf("fUniqueID:%d fUniqueIDStr:%x %x %x %x\n",fUniqueID,fUniqueIDStr.Ptr[0],fUniqueIDStr.Ptr[1],fUniqueIDStr.Ptr[2],fUniqueIDStr.Ptr[3]);
		#endif
		fRef.Set(fUniqueIDStr, this);
...
void EventContext::RemoveEvent(int event)
{
#ifdef __linux__
    select_removeevent_epoll(&fEventReq,event);
#endif
    return ;
}
    ...
    while (true)
    {
        ...
        while (theErrno == EINTR)
        {
            ...
            if(IsforSocket())
            {
                if(!IsUseEpoll())
                {
                    theReturnValue = select_waitevent(&theCurrentEvent, NULL);
                }
#ifdef __linux__                
                else
                {
                    //printf("using epoll\n");
                    theReturnValue = select_waitevent_epoll(&theCurrentEvent, NULL);
                    //printf("theReturnValue: %d\n",theReturnValue);
            ...
            {
#ifdef __linux__
                 theReturnValue = select_waitevent_hw(&theCurrentEvent, NULL);
#endif
            }
		  #ifdef PRINT
           printf("this: 0x%x now select return \n",this);
          #endif
#endif  
