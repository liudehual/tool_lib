...
class EventContext
{
    public:
        ...
        // remove events, just for epoll module add by gyd 
    ...
    protected:
		...
		virtual void ProcessEvent(int /*eventBits*/) //gyd add Task::kWriteEvent
        {   
            if (EVENTCONTEXT_DEBUG)
            {
                if (fTask== NULL)  
...
class EventThread : public OSThread
{
    public:
        ...
        virtual ~EventThread() {}
        void SetThreadFor(int type){ThreadFor = type;}
        int IsforSocket(){return !ThreadFor;}
        int IsUseEpoll(){return UseEpoll;}
    private:
